...
from .models import *
from .models import Market, Aborud, Mahsulotlar, Seryo
...
from .admin import BuyurtmaAdmin
def zakazlar_royxati(request):
    tasdiqlangan_zakazlar = Zakaz.objects.filter(tasdiqlandi=True)
    return render(request, 'zakazlar.html', {'zakazlar': tasdiqlangan_zakazlar})
...
def search(request):
    ...
    return render(request, 'search.html', {'query': query, 'search_obj': results , 'search_objs': resultss , 'search_objsa': resultsa , 'search_objsm': resultsm})
def saved(request):
...
def sklad_1(request):
    ...
    return render(request, 'sklad.html', {'sklad_1':sklad_1})
def home(request):
    markets = Market.objects.order_by('-title')
    advertisements = Advertisement.objects.all()
    return render (request, 'home.html', {'markets':markets , "advertisements":advertisements})
...
class Delete(DeleteView):
    ...
    success_url = reverse_lazy('uz_home')
...
def kimdirda(request, id):
    try:
        ...
        return redirect('uz_home')  # Xatolik yuz berganda asosiy sahifaga qaytadi
def aboruds_savatga_qoshish(request, id):
    try:
        aborud = Aborud.objects.get(id=id)
        aborud.sklad = True
        aborud.save()
        return redirect('sklad')  # Yoki kerakli sahifaga qaytish
    except Aborud.DoesNotExist:
        return redirect('uz_home')  # Agar topilmasa, asosiy sahifaga qaytish
def aboruds(request):
    aboruds = Aborud.objects.order_by('-title')
    print(aboruds)
    return render(request, 'aboruds.html', {'aboruds': aboruds})
...
def mahsulot_savatga_qoshish(request, id):
    try:
        ...
        mahsulot.save()
        return redirect('sklad')  # Savat sahifasiga qaytish
    except Mahsulotlar.DoesNotExist:
        return redirect('uz_home')  # Topilmasa asosiy sahifaga qaytish
...
def mahsulot_savatdan_ochir(request, id):
    try:
        ...
        return redirect('sklad')  # Agar mahsulot topilmasa, savatga qaytish
...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            return redirect('uz_home')  # Redirect to dashboard on success
    else:
...
def custom_logout(request):
    logout(request)
    return HttpResponseRedirect(reverse('uz_home'))
...
def buyurtma_berish(request):
    if not request.user.is_authenticated:
        messages.error(request, "Buyurtma berish uchun avval login qiling!")
    ...
    if request.method == 'POST':
        ...
        if not (market_sklad.exists() or seryo_sklad.exists() or aborud_sklad.exists() or mahsulot_sklad.exists()):
            ...
            return redirect('sklad')  # Sklad sahifasiga qaytarish
