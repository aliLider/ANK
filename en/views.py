...
def zakazlar_royxati(request):
    ...
    return render(request, 'en/zakazlar.html', {'zakazlar': tasdiqlangan_zakazlar})

def advertisements_page(request):
    advertisements = Advertisement.objects.all()
    return render(request, 'en/advertisements.html', {'advertisements': advertisements})
...
def search(request):
    ...
    return render(request, 'en/search.html', {'query': query, 'search_obj': results , 'search_objs': resultss , 'search_objsa': resultsa , 'search_objsm': resultsm})

def saved(request):
    # saqlash = Market.objects.all()
    saved = Market.objects.filter(saved=True)
    return render(request, 'en/saved.html', {'saved':saved})

def sklad(request):
    sklad = Market.objects.filter(sklad=True)
    return render(request, 'en/sklad.html', {'sklad':sklad})
def sklad_1(request):
    sklad_1 = Seryo.objects.filter(sklad_1=True)
    return render(request, 'en/sklad.html', {'sklad_1':sklad_1})
def home(request):
    markets = Market.objects.order_by('-title')
    advertisements = Advertisement.objects.all()
    return render (request, 'en/home.html', {'markets':markets , "advertisements":advertisements})

def category(request, id):
    rangi = Category.objects.get(id=id)
    markets = Market.objects.filter(rangi=rangi)
    content = {"category":rangi, 'markets':markets}
    return render(request, "en/category.html", content)
...
def categories(request):
    ...
    return render(request, "en/categories.html", content)
...
def aboruds(request):
    ...
    return render(request, 'en/aboruds.html', {'aboruds':aboruds})

def mahsulot(request):
    mahsulot = Mahsulotlar.objects.order_by('-title')
    return render(request, 'en/mahsulotlar.html', {'mahsulotlar':mahsulot})

def seryo(request):
    seryo = Seryo.objects.order_by('-title')
    return render(request, 'en/seryo.html', {'seryolar':seryo})

class Delete(DeleteView):
    model = Market
    template_name = 'en/delete.html'
    success_url = reverse_lazy('en_home')

class Edit(UpdateView):
    model = Market
    template_name = 'en/edit.html'
...
def nimadirda(request, id):
    ...
    market.saved = True
    market.save()
    return redirect('en_saved')
def minus(request, id):
    market = Market.objects.get(id=id)
    market.sklad = False
    market.save()
    return redirect('en_sklad')
...
def kimdirda_1(request, id):
    ...
    seryo.sklad_1= True
    seryo.save()
    return redirect('en_sklad_1')

def sklad(request):
    sklad = Market.objects.filter(sklad=True)
    return render(request, 'en_sklad.html', {'sklad': sklad})
def sklad(request):
    market_sklad = Market.objects.filter(sklad=True)
    seryo_sklad = Seryo.objects.filter(sklad_1=True)
    return render(request, 'en_sklad.html', {
...
def kimdirda(request, id):
    try:
        ...
        return redirect('en_home')  # Xatolik yuz berganda asosiy sahifaga qaytadi
def aboruds_savatga_qoshish(request, id):
    try:
        aborud = Aborud.objects.get(id=id)
        aborud.sklad = True
        aborud.save()
        return redirect('en_sklad')  # Yoki kerakli sahifaga qaytish
    except Aborud.DoesNotExist:
        return redirect('en_home')  # Agar topilmasa, asosiy sahifaga qaytish
def aboruds(request):
    aboruds = Aborud.objects.order_by('-title')
    print(aboruds)
    return render(request, 'en/aboruds.html', {'aboruds': aboruds})
...
def mahsulot_savatga_qoshish(request, id):
    try:
        ...
        mahsulot.save()
        return redirect('en_sklad')  # Savat sahifasiga qaytish
    except Mahsulotlar.DoesNotExist:
        return redirect('en_home')  # Topilmasa asosiy sahifaga qaytish
...
def sklad(request):
    ...
    return render(request, 'en/sklad.html', {
        'market_sklad': market_sklad,
        'seryo_sklad': seryo_sklad,
...
def mahsulot_savatdan_ochir(request, id):
    try:
        ...
        return redirect('en_sklad')  # Agar mahsulot topilmasa, savatga qaytish
...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            return redirect('en_home')  # Redirect to dashboard on success
    else:
        form = CustomUserCreationForm()
    return render(request, 'en/register.html', {'form': form})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                return render(request, 'en/login.html', {'form': form, 'error': 'Invalid credentials'})
        else:
            return render(request, 'en/login.html', {'form': form, 'error': 'Invalid form submission'})
    else:
...
def dashboard_view(request):
    ...
    return render(request, 'en/dashboard.html', {
...
def custom_logout(request):
    logout(request)
    return HttpResponseRedirect(reverse('en_home'))
...
def buyurtma_berish(request):
    if request.method == 'POST':
        ...
        if not (market_sklad.exists() or seryo_sklad.exists() or aborud_sklad.exists() or mahsulot_sklad.exists()):
            ...
            return redirect('en_sklad')  # Sklad sahifasiga qaytarish
